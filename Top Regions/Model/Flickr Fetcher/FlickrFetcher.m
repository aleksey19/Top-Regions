...
{
    return [self URLForQuery:[NSString stringWithFormat:@"https://api.flickr.com/services/rest/?method=flickr.places.getInfo&api_key=%@&place_id=%@", FlickrAPIKey, flickrPlaceId]];
}
...
+ (NSString *)extractNameOfPlace:(id)placeId fromPlaceInformation:(NSDictionary *)place
{
    ...
    NSLog(@"%@", [place valueForKeyPath:FLICKR_PLACE_NEIGHBORHOOD_PLACE_ID]);
    NSLog(@"%@", [place valueForKeyPath:FLICKR_PLACE_LOCALITY_PLACE_ID]);
    NSLog(@"%@", [place valueForKeyPath:FLICKR_PLACE_COUNTY_PLACE_ID]);
    NSLog(@"%@", [place valueForKeyPath:FLICKR_PLACE_REGION_PLACE_ID]);
    NSLog(@"%@", [place valueForKeyPath:FLICKR_PLACE_COUNTRY_PLACE_ID]);
